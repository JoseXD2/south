...
class CutsceneDialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		var hasDialog=true;
		box.loadGraphic(Paths.image('cutscenes/pene'));

		box.updateHitbox();
		this.dialogueList = dialogueList;

		nametag = new FlxSprite(Std.int(FlxG.width/2), 40);
		nametag.loadGraphic(Paths.image('cutscenes/cartelBF'));
		...
		dropText.font = 'Questrian';
		dropText.color = FlxColor.WHITE;
		add(dropText);

		swagDialogue = new FlxTypeText(240, 500, Std.int(FlxG.width * 0.6), "", 32);
		swagDialogue.font = 'Questrian';
	...
	function startDialogue():Void
	{
		...
		switch(curCharacter){
			case 'mysterion':
				nametag.loadGraphic(Paths.image('cutscenes/cartelMS'));
				nametag.updateHitbox();
				nametag.scrollFactor.set();
				nametag.screenCenter(XY);
				dropText.color = 0xFF271838;
				swagDialogue.sounds = [FlxG.sound.load(Paths.sound('dialogue/mysterion'), 0.6)];
			case 'bf':
				nametag.loadGraphic(Paths.image('cutscenes/cartelBF'));
				nametag.updateHitbox();
				nametag.scrollFactor.set();
