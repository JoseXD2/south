...
class PlayState extends MusicBeatState
{
	...
	var flashSprite:FlxSprite = new FlxSprite(-70, -70).makeGraphic(5000, 5000, 0xFFFFFFFF);
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
            ...
            case 'chips':
				dialogue = CoolUtil.coolTextFile(Paths.txt('chips/dialog'));
			case 'hot-dog':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hot-dog/dialog'));
    		case 'burger':
				dialogue = CoolUtil.coolTextFile(Paths.txt('burger/dialog')); 
			case 'pizza':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pizza/dialog'));
		}
		...
		if (SONG.stage == null) {
			switch(storyWeek)
			{
				...
				case 7: stageCheck = 'chips';
				case 8: stageCheck = 'hot-dog';
				case 9: stageCheck = 'burger';
				case 10: stageCheck = 'pizza';
		...
		switch(stageCheck)
		{
			...
			case 'burger':
			{
					defaultCamZoom = 0.85;
					curStage = 'week7';
					var bg:FlxSprite = new FlxSprite(-500, -180).loadGraphic(Paths.image('weeb/bus3'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.95, 0.95);
					bg.active = false;
					add(bg);
		...
		if (curStage == 'sas')
		{
			...
			lluvia.frames = Paths.getSparrowAtlas('sas/lluviendo');
			lluvia.setGraphicSize(Std.int(lluvia.width * 1.7));
			lluvia.animation.addByPrefix('idle', "lluvia 2", 24);
			lluvia.animation.play('idle');
			lluvia.scrollFactor.set(1.5, 1.5);
			lluvia.alpha = 0.3;
			add(lluvia);
		}
	...
	function newIntro(?dialogueBox:DialogueBox):Void
	{
		...
		senpaiEvil.frames = Paths.getSparrowAtlas('weeb/kennyd');
		senpaiEvil.animation.addByPrefix('idle', 'despiste 2', 24, false);
		...
		FlxTween.tween(white, {alpha: 0}, 2.5, {
		    onComplete: function(twn:FlxTween)
            {
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					case 'sas':
						babyArrow.frames = Paths.getSparrowAtlas('sas/NOTE_assets');
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if(SONG.song.toLowerCase()=='pizza' || FlxG.save.data.moveNotes ){
						for(str in playerStrums){
							str.angle = 10*Math.cos((timer*2)+str.ID*2);
							str.y = strumLine.y+(0*Math.sin((timer*2)+str.ID*2));
							str.x = str.health+(80*Math.cos(((timer*4)+str.ID/2)));
						}
					}
					if(SONG.song.toLowerCase()=='pizza' || FlxG.save.data.moveNotes){
                        if(daNote.mustPress){
							daNote.angle = 10*Math.cos((timer*2)+daNote.ID*2);
                            daNote.y = daNote.y+(0*Math.sin((timer*2)+daNote.noteData*2));
                            daNote.x = daNote.health+(80*Math.cos(((timer*4)+daNote.noteData/2)));
                        }
                    }
	...
	*/
	...
	override function stepHit()
	{
		...
		//var LOLFlashes = [1909];
		//var _g = 0;
		//while(_g < LOLFlashes.length) {
		//	var LOLflash = LOLFlashes[_g];
		//	++_g;

		if (curStep == 1912 && curSong == 'burger')
            {
                dad.playAnim('despiste', false);
            }

        if (curStep == 1849 && curSong == 'pizza')
            {
                dad.playAnim('rayo', false);
            }
	...
	override function beatHit()
	{
		...
		if(curSong == 'burger') // LOL flashes
		{
			
			if(curBeat == 1909)
				_cb = 1;
				if(curBeat > 1909 && curBeat < 1930) 
				{
					_cb++;
					if(_cb == 2)
					{
			...
			while(_b < LOLBeats.length) {
			var LOLflash = LOLBeats[_b];
				++_b;
				if(curBeat == LOLflash)
				{
					bgFlash();
